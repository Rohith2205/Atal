...
class AuthController extends GetxController {
  ...
  Future<UserStatus> _checkUserStatus() async {
    try {
      ...
      final isPolicyAccepted = _userController.isPolicyAccepted.value;

      safePrint('Personal details completed: $hasPersonalDetails, Policy accepted: $isPolicyAccepted');
    ...
    try {
      ...
      if (session.isSignedIn) {
        ...
        if (userStatus == UserStatus.needsPersonalDetails && !_hasCompletedPersonalDetailsFlow.value) {
          isNewUser.value = true;
          // Only show dialog if we're in the right context (not during initialization)
          if (!isLoading.value) {
            _showPersonalDetailsDialog();
          }
        } else {
